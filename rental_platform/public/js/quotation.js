frappe.ui.form.on('Quotation', {
    validate: function (frm) {
        ...
        frm.doc.custom_rental_items.forEach(row => {
...
frappe.ui.form.on('Booking Details', {
    ...
    rental_item_id: function (frm, cdt, cdn) {
        ...
        if (child.rental_item_id) {
            frappe.db.get_value('Item', child.rental_item_id, 'item_name', r => {
                if (r && r.item_name) {
                    frappe.model.set_value(cdt, cdn, 'item_name', r.item_name);
                }
            });
            if (frm.doc.pricelist_name) {
                frappe.call({
                    ...
                    callback: function (data) {
                        if (data.message) {
                            ...
                            frappe.msgprint(__('No price found for this item in the selected pricelist.'));
                        }
                    }
                });
            }
